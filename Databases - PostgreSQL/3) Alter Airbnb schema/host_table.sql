--Creating Host table with data from Listings
CREATE TABLE Host AS 
	SELECT DISTINCT host_id, host_url, host_name, host_since,host_location, host_about, host_response_time, host_response_rate, host_acceptance_rate,
...
--Renaming the columns of table Host
...
ALTER TABLE Listings DROP COLUMN host_url,DROP COLUMN host_name, DROP COLUMN host_since, DROP COLUMN host_location,DROP COLUMN host_about,DROP COLUMN host_response_time, DROP COLUMN host_response_rate, DROP COLUMN host_acceptance_rate,
...
--Setting Host table primary key
ALTER TABLE Host ADD PRIMARY KEY (id);
--Setting Listings table foreign key
